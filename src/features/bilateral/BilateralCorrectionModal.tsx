...
export function BilateralCorrectionModal({
  visible,
  onClose,
  originalEntry,
  ...
  const mutation = useMutation({
    mutationFn: createBilateralCorrection,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['bilateral-pairs'] });
      queryClient.invalidateQueries({ queryKey: ['bilateral-pair-detail'] });
      Alert.alert(
        'Correction Recorded',
        'Bilateral reversal and adjustment entries have been recorded.'
      );
      resetForm();
      onClose();
    },
    onError: (err: any) => {
      Alert.alert('Correction Failed', err?.message || 'Could not record bilateral correction.');
    },
  });
    ...
    <Modal visible={visible} animationType="slide" transparent onShow={handleOpen} onRequestClose={onClose}>
      <View style={styles.overlay}>
        <View style={styles.container}>
          ...
          <Text style={styles.title}>Correct Pair Expense</Text>
          <Text style={styles.subtitle}>
            Original: {originalEntry?.description} ({originalEntry ? formatInr(originalEntry.amountPaise) : ''})
          </Text>
          <Text style={styles.notice}>
            Appends a BILATERAL_REVERSAL and a new BILATERAL_ADJUSTMENT to preserve ledger immutability.
          </Text>
          ...
          <View style={styles.form}>
            ...
            <TextInput
              style={styles.input}
              placeholder="Food, rent, personal…"
              value={category}
